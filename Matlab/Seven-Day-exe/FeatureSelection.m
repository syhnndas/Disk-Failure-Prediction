...
topN = 50;
sorted_features = sorted_data.Features(1:min(topN, height(sorted_data)));
sorted_normalized = sorted_data{1:min(topN, height(sorted_data)), 2:5};
figure();
...
% ===== 🔥 排序（关键） =====
total_score = sum(sorted_normalized, 2);
[~, idx] = sort(total_score, 'descend');

sorted_normalized = sorted_normalized(idx, :);
sorted_features = sorted_features(idx);

% ===== 绘图 =====
figure('Position',[100 100 700 600]);

barh(sorted_normalized, 'stacked');

ax = gca;
ax.YTick = 1:length(sorted_features);
...
xlabel('Normalized Importance Score', 'FontSize', 14);
ylabel('Features', 'FontSize', 14);

legend(col_names,'Location','northoutside','Orientation','horizontal');
