...
% data = data_raw(randsample(find(T_raw.failure==0),20,false), :);
idx_pool = find(T_raw.failure == 0);
while true
    idx = randsample(idx_pool, 20, false);
    data = data_raw(idx, :);

    % 统计“全零列”的数量
    num_zero_cols = sum(all(data == 0, 1));
...
% idx_pool = find(T_raw.failure == 0);
% while true
%     idx = randsample(idx_pool, 50, false);
%     data = data_raw(idx, :);
% 
%     % 判断是否存在“全零列”
%     col_all_zero = all(data == 0, 1);
% 
%     if ~any(col_all_zero)
%         break;  % 满足条件就退出
%     end
...
xlabel('Normalized Value', 'FontName', 'Times New Roman', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('Features', 'FontName', 'Times New Roman', 'FontSize', 14, 'FontWeight', 'bold');
% % --- 6.保存图像 ---
